...
YouTube 수집에는 워크플로우 안에서 `yt-dlp`를 설치해서 검색어별 관련 채널을 찾고, YouTube 공식 채널 RSS로 최근 업로드와 Shorts 링크를 확인한 뒤, 가능한 경우 공개 메타데이터로 길이와 조회수를 보강합니다. Instagram/Threads 고품질 수집은 공개 공급자 API가 있을 때만 자동 병합합니다. API가 없거나 공개 페이지에서 지표를 확인할 수 없는 항목은 랭킹 데이터에 넣지 않고 `data/public-candidates.json` 후보로만 남깁니다.
...
품질 기준: YouTube 직접 수집은 단순 검색 결과를 모두 넣지 않습니다. 1차 `급상승` 기준은 최근 45일 안의 Shorts/RSS 검증 영상 중 조회수 3만 이상이거나 일평균 조회수 1만 이상인 항목입니다. 2차 `신선후보` 기준은 최근 10일 안의 Shorts/RSS 검증 영상 중 조회수 1천 이상이거나 일평균 조회수 300 이상인 항목입니다. `급상승`은 검색어별 최대 5개, `신선후보`는 검색어별 최대 15개이면서 전체 최대 120개까지 받습니다. 공개 메타데이터로 길이가 확인되는 경우에는 180초 이하 조건도 함께 확인합니다. 통과한 항목에는 `dailyViews`, `trendScore`, `qualityReason`, `signalLabel`, `tier`, `sourceQuery`, `collectedAt`, `evidence`가 저장됩니다. Instagram/Threads 공개 페이지에서 조회수나 업로드일을 확인할 수 없는 항목은 기본적으로 자동 추가하지 않고 수집 상태의 `skipped`에 기록합니다.

최근 확인 결과: 2026-06-10 08:15 KST 수동 검증 실행에서 YouTube는 22개 검색어, 관련 채널 80개, 최근 후보 194개를 검사했고 RSS/공개 메타데이터 기반 후보 140개 중 85개가 Shorts 링크와 조회수 기준을 통과했습니다. 최종 랭킹 후보는 `급상승` 23개와 `신선후보` 47개, 총 70개였고 아직 없던 YouTube 항목 52개를 새로 추가했습니다. 이번 수정 작업 전체로 YouTube 데이터는 2,099개에서 2,175개로 76개 늘었습니다. Instagram은 공급자 API 비밀값이 없어 0개, Threads는 무조회수 공개 후보 72개를 후보 파일에만 저장했습니다.
